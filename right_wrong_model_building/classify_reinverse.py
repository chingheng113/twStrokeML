...
data = data.assign(MRS_TX_3=pd.cut(data['MRS_TX_3'], [-1, 2, 7], labels=[0, 1]).astype(int))
data = data.assign(MRS_TX_1=pd.cut(data['MRS_TX_1'], [-1, 2, 7], labels=[0, 1]).astype(int))
data = data.drop_duplicates(['ICASE_ID', 'IDCASE_ID'])

y_data = pd.DataFrame({'reinverse': np.abs(data.MRS_TX_3 - data.MRS_TX_1)})
print(y_data.shape)
...
for i in range(1):
    ...
    # over-sampling
    print('before', y_train.groupby(['reinverse']).size())
    ...
    print('after', y_train.groupby(['reinverse']).size())
    ...
    W.to_csv('reinvers_explain.csv', index=False)
    print('done')